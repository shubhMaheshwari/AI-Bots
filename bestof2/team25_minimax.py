...
class Team25_minimax():
	...
	def check_block_value(self,x,y,ply):	
		...
		#checking for diamond pattern
		cnt_ply = self.check_diamond(x+1,y+1,ply)
		if cnt_ply == 16:
			return cnt_ply
		elif cnt_ply == -16:
			return cnt_ply 	
		...
		cnt_ply = self.check_diamond(x+1,y+2,ply)
		if cnt_ply == 16:
			return cnt_ply
		elif cnt_ply == -16:
			return cnt_ply 	
		...
		cnt_ply = self.check_diamond(x+2,y+1,ply)
		if cnt_ply == 16:
			return cnt_ply
		elif cnt_ply == -16:
			return cnt_ply 	
		...
		cnt_ply = self.check_diamond(x+2,y+2,ply)
		if cnt_ply == 16:
			return cnt_ply
		elif cnt_ply == -16:
			return cnt_ply 	
	...
	def check_diamond(self,x,y,ply):
		bs = self.board.board_status
	...
	def block_check_diamond(self,ind,ply):
		...
		if ind == 1:	
			...
			return cnt_ply*(1 if ply == 'x' else -1)		
	...
	# 	with open('depth.json','w+') as d:
	# 		json.dump(self.hash_depth,d)
	...
	# 			print("Using Already stored Values")
		
	# 	else:
	# 		self.hash_depth = {}
	# 		self.hash_table = {}
	# 		print("No stored values")

	# 	if os.path.exists('./depth.json'):
	# 		with open('depth.json','r') as d:
	# 			json.load(d)
