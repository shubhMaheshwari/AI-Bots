...
class Team25_minimax():
	...
	def move(self, board, old_move, flag):
		...
		print("Total time:",new_time - self.time)	
	...
	def min_max(self, old_move,ply,depth,alpha = -10000,beta = 10000):		
		...
		if(depth == 0):
			return old_move, self.eval_board()
			# return old_move,0
	...
	def eval_board(self):
		...
		for i in range(4):
			for j in range(4):
				if bs.block_status[i][j] == 'x':
					x += 1
				if bs.block_status[i][j] == 'o':
					o += 1
				if bs.block_status[i][j] == 'd':
					d += 1

		return (x - o)/(d+1) # make sure the d + 1 != 0			
